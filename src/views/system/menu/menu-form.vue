...
<template>
  <n-form ref="dataForm" :rules="rules" :model="temp" label-placement="left" label-width="100px">
    ...
    <n-form-item label="打开方式" path="openMode" v-if="menuType == 'menu' && (temp.url.startsWith('http') || (temp.url.startsWith('/') && temp.url.indexOf('.htm') != -1))">
      <n-radio-group v-model:value="openModeRef">
        <n-radio-button key="0" value="0" label="iframe" />
        <n-radio-button key="1" value="1" label="新标签页" />
      </n-radio-group>
    </n-form-item>
    <n-form-item label="关联组件" path="componentName" v-if="menuType == 'menu' && !(temp.url.startsWith('http') || (temp.url.startsWith('/') && temp.url.indexOf('.htm') != -1))">
      <n-tree-select
