...
<template>
  <div class="mb-list">
        ...
        <n-grid :cols="24">
          <n-gi :span="12">
            <n-form-item label="菜单权限" path="menus">
              <mb-tree ref="tree" v-model="temp.menus" style="height: 270px; overflow: auto" url="/system/menu/tree" :search="true" />
            </n-form-item>
          </n-gi>
          <n-gi :span="12">
            <n-form-item label="数据权限" path="permission">
              <mb-select v-model="temp.permission" :options="permissionData" style="margin-bottom: 5px" />
              <mb-tree v-if="temp.permission == 1" style="height: 270px; overflow: auto;" ref="office" url="/system/office/tree" v-model="temp.offices" />
            </n-form-item>
          </n-gi>
        </n-grid>
      </n-form>
    ...
    <mb-modal ref="assignPermissionsDialog" title="分配权限" width="550px" @confirm="assignPermissions.save($event)">
