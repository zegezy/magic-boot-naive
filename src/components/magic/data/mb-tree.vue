<template>
  <div>
    <div style="margin-bottom: 5px;" v-if="expand || checked">
      <n-space>
        <n-button v-if="expand" type="primary" @click="doExpand">展开/折叠</n-button>
        <n-button v-if="checked" type="primary" @click="() => { treeAllChecked = !treeAllChecked; checkedAll(treeAllChecked) }">全选/全不选</n-button>
      </n-space>
    </div>
    <div style="margin-bottom: 5px;" v-if="search">
      <n-input v-model:value="searchValue" placeholder="输入关键字进行过滤" />
    </div>
    <n-tree
      v-bind="props.props"
      virtual-scroll
