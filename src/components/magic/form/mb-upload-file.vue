<template>
  ...
  >
    <n-upload-dragger v-if="directoryDnd">
      <div>可拖拽上传</div>
      <n-button type="primary" :loading="uploadLoading" :disabled="!multiple && fileList.length == 1">{{ label }}</n-button>
...
<script setup>
  ...
  onMounted(() => {
    if (props.externalId) {
      common.$get('/system/file/files', { externalId: props.externalId, externalType: props.externalType }).then(res => {
        const { data } = res
        fileList.value = data
      })
